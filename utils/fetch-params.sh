...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/KeymakerParams"
else
    PARAMS_DIR="$HOME/.keymaker-params"
fi
...
DOWNLOAD_URL="https://github.com/keymakercoin/keymaker/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with KEYM_DISABLE_SOMETHING=1
KEYM_DISABLE_WGET="${KEYM_DISABLE_WGET:-}"
#KEYM_DISABLE_IPFS="${KEYM_DISABLE_IPFS:-}"
KEYM_DISABLE_CURL="${KEYM_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$KEYM_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$KEYM_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$KEYM_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Keymaker parameters!
...
main() {
    ...
    cat <<EOF
KEYM - fetch-params.sh

This script will fetch the Keymaker SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Keymaker SNARK parameters. Note that it is
