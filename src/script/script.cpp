...
bool CScript::IsAssetScript(int &nIndex) const {
    if (this->size() > 31) {
        if ((*this)[25] == OP_ASSET_ID) { // OP_KEYM_ASSET is always in the 25 index of the script if it exists
            nIndex = -1;
            if ((*this)[27] == KEYM_R) { // Check to see if KEYM starts at 27 ( this->size() < 105)
                if ((*this)[28] == KEYM_T)
                    if ((*this)[29] == KEYM_M)
                        nIndex = 30;
            } else {
                if ((*this)[28] == KEYM_R) // Check to see if KEYM starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == KEYM_T)
                        if ((*this)[30] == KEYM_M)
                            nIndex = 31;
            }
