...
static int AppInitRPC(int argc, char *argv[]) {
    ...
    if (argc < 2 || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\nUsage:\n"
                        "  keymaker-cli [options] <command> [params]  " +
                        strprintf("Send command to %s", PACKAGE_NAME) + "\n" +
                        "  keymaker-cli [options] -named <command> [name=value] ... " +
                        strprintf("Send command to %s (with named arguments)", PACKAGE_NAME) + "\n" +
                        "  keymaker-cli [options] help                List commands\n" +
                        "  keymaker-cli [options] help <command>      Get help for a command\n";
...
static UniValue CallRPC(BaseRequestHandler *rh, const std::string &strMethod, const std::vector <std::string> &args) {
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf(
                "Could not connect to the server %s:%d%s\n\nMake sure the keymakerd server is running and that you are connecting to the correct RPC port.",
