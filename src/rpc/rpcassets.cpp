// Copyright (c) 2023 The Keymaker developers
...
UniValue sendasset(const JSONRPCRequest &request) {
    if (request.fHelp || !Updates().IsAssetsActive(::ChainActive().Tip()) || request.params.size() < 3 ||
        ...
        throw std::runtime_error(
                ...
                "4. \"change_address\"          (string, optional, default = \"\") the transactions KEYM change will be sent to this address\n"
    ...
    if (curBalance == 0) {
        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS,
                           "Error: This wallet doesn't contain any KEYM, transfering an asset requires a network fee");
    }
    ...
    if (!IsValidDestination(to_dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Keymaker address: ") + to_address);
    }
    ...
    if (!change_address.empty() && !IsValidDestination(change_dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER,
                           std::string("KEYM change address must be a valid address. Invalid address: ") +
                           change_address);
...
UniValue listunspentassets(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "    \"account\" : \"account\",    (string) DEPRECATED. This field will be removed in V0.18. To see this deprecated field, start keymakerd with -deprecatedrpc=accounts. The associated account, or \"\" for the default account\n"
...
UniValue listassetbalancesbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !Updates().IsAssetsActive(::ChainActive().Tip()) || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"                  (string, required) a keymaker address\n"
    ...
    if (!IsValidDestination(destination)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Keymaker address: ") + address);
    }
