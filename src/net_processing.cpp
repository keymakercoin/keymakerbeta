 ...
 *  We have 4 times smaller block times in Keymaker, so we need to push 4 times more invs per 1MB. */
...
EXCLUSIVE_LOCKS_REQUIRED(cs_main)
        {
                switch (inv.type)
                {
                    ...
                    /*
                        Keymaker Related Inventory Messages
...
bool PeerLogicValidation::SendMessages(CNode *pto) {
    const Consensus::Params &consensusParams = Params().GetConsensus();
    {
        ...
        // KEYMAKER this code also handles non-TXs (Dash specific messages)
