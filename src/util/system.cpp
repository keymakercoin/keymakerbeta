...
public:
    ...
    static inline std::pair<bool, std::string> GetArg(const ArgsManager &am, const std::string &arg) {
        ...
        // argument value seen from the command line (so "keymakerd -foo=bar
...
fs::path GetDefaultDataDir() {
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KeymakerCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KeymakerCore
    // Mac: ~/Library/Application Support/KeymakerCore
    // Unix: ~/.keymakercore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KeymakerCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KeymakerCore";
#else
    // Unix
    return pathRet / ".keymakercore";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string &error, bool ignore_invalid_keys) {
    ...
    if (stream.good()) {
        ...
        // Create an empty keymaker.conf if it does not exist
...
std::string CopyrightHolders(const std::string &strPrefix, unsigned int nStartYear, unsigned int nEndYear) {
    ...
    // Check for untranslated substitution to make sure Keymaker Core copyright is not removed by accident
    if (copyright_info.find("Keymaker") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2020, nEndYear) + "The Keymaker developers";
    }
