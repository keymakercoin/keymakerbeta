...
static bool AppInit(int argc, char *argv[]) {
    ...
    // Parameters
    //
    // If Qt is used, parameters/keymaker.conf are parsed in qt/keymaker.cpp's main()
    SetupServerArgs();
    std::string error;
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(strprintf(
                        "Error: Command line contains unexpected token '%s', see keymakerd -h for a list of options.\n",
                        argv[i]));
            }
        }

        // -server defaults to true for keymakerd but not for the GUI so do this here
