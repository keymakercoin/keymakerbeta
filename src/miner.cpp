...
void static KeymakerMiner(const CChainParams& chainparams, NodeContext& node) {
    LogPrintf("KeymakerMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("keymaker-miner");
    ...
    unsigned int nExtraNonce = 0;
        ...
        //     LogPrintf("KeymakerMiner -- Wallet not available\n");
        // }
    #endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                LogPrintf(
                        "KeymakerMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("KeymakerMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
int GenerateKeymakers(bool fGenerate, int nThreads, const CChainParams &chainparams, NodeContext &node) {
    ...
    for (int i = 0; i < nThreads; i++) {
        minerThreads->create_thread(
                boost::bind(&KeymakerMiner, boost::cref(chainparams), boost::ref(node)));
    }
    return (numCores);
}