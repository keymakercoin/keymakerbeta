...
void OptionsModel::Init(bool resetSettings) {
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::KEYM);
    ...
    if (!settings.contains("nCoinJoinAmount")) {
        ...
        if (!settings.contains("nAnonymizeKeymakerAmount"))
            settings.setValue("nCoinJoinAmount", DEFAULT_COINJOIN_AMOUNT);
        else
            settings.setValue("nCoinJoinAmount", settings.value("nAnonymizeKeymakerAmount").toInt());
    }
