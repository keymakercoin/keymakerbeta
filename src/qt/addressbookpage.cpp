    ...
    switch (tab) {
        case SendingTab:
            ui->labelExplanation->setText(
                    tr("These are your Keymaker addresses for sending payments. Always check the amount and the receiving address before sending coins."));
            ui->deleteAddress->setVisible(true);
            ui->newAddress->setVisible(true);
            break;
        case ReceivingTab:
            ui->labelExplanation->setText(
                    tr("These are your Keymaker addresses for receiving payments. It is recommended to use a new receiving address for each transaction."));
...
void AddressBookPage::on_showAddressQRCode_clicked() {
    ...
    dialog->setInfo(tr("QR code"), "keymaker:" + strAddress, "", strAddress);
    dialog->show();
}
...
void AddressBookPage::on_exportButton_clicked() {
    ...
    if (!writer.write()) {
        //: %1 is a name of the file (e.g.: "addressbook.csv") that the Keymaker addresses were exported to.
