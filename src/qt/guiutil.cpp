...
// Contains a list of supported font weights for all members of GUIUtil::FontFamily
    ...
    static const std::map <ThemedColor, QColor> themedColors = {
            ...
            {ThemedColor::ORANGE,                  QColor(99, 89, 243)},
...
// Generate a dummy address with invalid CRC, starting with the network prefix.
    ...
    void setupAppearance(QWidget *parent, OptionsModel *model) {
        if (!QSettings().value("fAppearanceSetupDone", false).toBool()) {
            ...
            dlg.setWindowIcon(QIcon(":icons/keymaker"));
    ...
    bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out) {
        // return if URI is not valid or is no keymaker: URI
        if (!uri.isValid() || uri.scheme() != QString("keymaker"))
            return false;
        ...
        i++)
        {
            ...
            if (i->first == "message") {
                ...
                if (!i->second.isEmpty()) {
                    if (!BitcoinUnits::parse(BitcoinUnits::KEYM, i->second, &rv.amount)) {
                        return false;
                    }
                }
                fShouldReturnFalse = false;
    ...
    QString formatBitcoinURI(const SendCoinsRecipient &info) {
        ...
        if (info.amount) {
            ret += QString("?amount=%1").arg(
                    BitcoinUnits::format(BitcoinUnits::KEYM, info.amount, false, BitcoinUnits::separatorNever));
            paramCount++;
        }
    ...
    void openConfigfile() {
        ...
        /* Open keymaker.conf with the associated application */
 ...
 */
    ...
    fs::path static StartupShortcutPath()
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            return GetSpecialFolderPath(CSIDL_STARTUP) / "Keymaker Core.lnk";
        if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
            return GetSpecialFolderPath(CSIDL_STARTUP) / "Keymaker Core (testnet).lnk";
        return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Keymaker Core (%s).lnk", chain);
    }

    bool GetStartOnSystemStartup()
    {
        // check for "Keymaker Core*.lnk"
    ...
    fs::path static GetAutostartFilePath()
    {
        std::string chain = gArgs.GetChainName();
        if (chain == CBaseChainParams::MAIN)
            return GetAutostartDir() / "keymakercore.desktop";
        return GetAutostartDir() / strprintf("keymakercore-%s.desktop", chain);
    }
    ...
    bool SetStartOnSystemStartup(bool fAutoStart)
    {
        ...
        else
        {
            ...
            if (!optionFile.good())
                return false;
            std::string chain = gArgs.GetChainName();
            // Write a keymakercore.desktop file to the autostart directory:
            optionFile << "[Desktop Entry]\n";
            optionFile << "Type=Application\n";
            if (chain == CBaseChainParams::MAIN)
                optionFile << "Name=Keymaker Core\n";
            else
                optionFile << strprintf("Name=Keymaker Core (%s)\n", chain);
    ...
    void disableMacFocusRect(const QWidget *w) {
        ...
        for (const auto& c : w->findChildren<QWidget*>()) {
            if (c->testAttribute(Qt::WA_MacShowFocusRect)) {
                c->setAttribute(Qt::WA_MacShowFocusRect, !keymakerThemeActive());
    ...
    void updateMacFocusRects() {
        ...
        while (it != setRectsDisabled.end()) {
            if (allWidgets.contains(*it)) {
                (*it)->setAttribute(Qt::WA_MacShowFocusRect, !keymakerThemeActive());
                ++it;
            } else {
