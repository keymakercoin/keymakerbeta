This directory contains the KeymakerQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Keymaker UI.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a keymaker:... link from a browser).
...
* don't change `local/keymaker_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "keymaker-qt" under "Run", which is where you can also add command line arguments)
