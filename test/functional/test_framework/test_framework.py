...
class BitcoinTestFramework():
    ...
    def main(self):
        ...
        parser.add_option("--keymakerd-arg", dest="dashd_extra_args", default=[], type='string', action='append',
                          help="Pass extra args to all keymakerd instances")
        ...
        self.config = config
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/keymakerd' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/keymaker-cli' + config["environment"]["EXEEXT"])
...
class KeymakerTestFramework(BitcoinTestFramework):
    def set_keymaker_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False):
...
def skip_if_no_bitcoind_zmq(test_instance):
    """Skip the running test if keymakerd has not been compiled with zmq support."""
    config = configparser.ConfigParser()
    config.read_file(open(test_instance.options.configfile))

    if not config["components"].getboolean("ENABLE_ZMQ"):
        raise SkipTest("keymakerd has not been built with zmq enabled.")
