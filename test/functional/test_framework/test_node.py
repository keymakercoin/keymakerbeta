...
class TestNode():
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'keymakerd exited with status {} during initialization'.format(self.process.returncode)))
            try:
        ...
        self._raise_assertion_error("Unable to connect to keymakerd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, partial_match=False, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to keymakerd
        expected_msg: regex that stderr should match when keymakerd fails

        Will throw if keymakerd starts without an error.
        Will throw if an expected_msg is provided and it does not match keymakerd's stdout."""
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                self.log.debug('keymakerd failed to start: %s', e)
                self.running = False
                self.process = None
