...
- [Development guidelines](#development-guidelines)
    - [General Keymaker Core](#general-keymaker-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
The Keymaker uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Keymaker Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/keymakercoin/keymaker/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_keymaker
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_raptoreu --log_level=test_suite
$ valgrind -v --leak-check=full src/keymakerd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_keymaker.coverage/index.html`.
```

**Sanitizers**

Keymaker Core can be compiled with various "sanitizers" enabled, which add
...
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_keymaker
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_keymaker --log_level=test_suite
$ valgrind -v --leak-check=full src/keymakerd -printtoconsole
```
...
However, in open source software such as Keymaker Core, where everyone uses
...
pay attention to for reviewers of Keymaker Core code.

General Keymaker Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Keymaker Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Keymaker Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `keymaker-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `keymaker-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Keymaker Core, the wallet was always
