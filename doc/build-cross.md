Cross-compiliation of Keymaker Core
===============================

Keymaker Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Keymaker.
...
When building Keymaker Core, use
...
Acquire the source in the usual way:
    ...
    git clone https://github.com/keymakercoin/keymaker
    cd keymaker
...
Note that for WSL the Keymaker Core source path MUST be somewhere in the default mount file system, for
example /usr/src/keymaker, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
It's not possible to build the Keymaker Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Keymaker Core source code).
