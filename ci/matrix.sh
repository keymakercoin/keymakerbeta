...
export BUILDER_IMAGE_NAME="keymaker-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_KEYMAKER_DEBUG"
  export PYZMQ=true
  export RUN_UNITTESTS=true
  export RUN_INTEGRATIONTESTS=true
