...
# keymaker_hash
RUN git clone https://github.com/raptor3um/keymaker_hash
RUN cd keymaker_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} keymaker
RUN useradd -u ${USER_ID} -g keymaker -s /bin/bash -m -d /keymaker keymaker
...
RUN mkdir /keymaker-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /keymaker-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /keymaker-src
